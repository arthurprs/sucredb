...
impl fmt::Debug for Value {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match *self {
            ...
            Value::Status(ref v) => write!(f, "Status({:?})", v.as_ref()),
            Value::Error(ref v) => write!(f, "Error({:?})", v.as_ref()),
        }
    }
}
...
impl RequestBuffer {
    ...
    pub fn mut_bytes(&mut self) -> &mut [u8] {
        ...
        if cur_len - self.bytes_read < MIN_REQUESTBUFFER_SIZE {
            unsafe {
                self.body.push_uninitialized(MIN_REQUESTBUFFER_SIZE);
            }
        }
...
mod tests {
    ...
    fn parse_valid_array() {
        ...
        assert_eq_repr!(r.unwrap(),
                        Value::Array(vec![Value::Data(b"foo".as_ref().into()),
